...
1. 将本地模型健康检查结果写入 AgentTrace，形成可回放的模型运行诊断记录。
2. 为 App 增加更明确的模型状态 UI：模型文件就绪、运行时已加载、推理成功、云端兜底中、本地失败原因。
3. 保持 Qwen-VL-Max 作为本地推理不可用、超时或结果不稳定时的云端兜底。
4. 在用户授予无障碍、悬浮窗和屏幕截图权限后，进行完整真机任务走查。
5. 继续强化微信草稿状态机，保证只进入草稿，不自动点击最终发送。
6. 扩展 App 专项策略库，优先覆盖浏览器、系统设置、美团、电话、微信。
7. 继续把失败日志、UI 树、截图状态、模型输出、执行结果统一沉淀到 AgentTrace。
8. 后续推进多 Agent 协作，包括规划 Agent、视觉/感知 Agent、执行 Agent、反思 Agent、安全 Agent。
